...
#             name_gu = gu[i]
    ...
    #새로운 데이터 모델에 삽입
    new_data = pd.DataFrame({'gu':[gu], 'exclusive_use_area':[exclusive_use_area], 'year_of_completion':[year_of_completion], 'transaction_year_month':[transaction_year_month], \
                 'floor':[floor], 'park_area_sum':[park_area_sum], 'day_care_babyTeacher_rate':[day_care_babyTeacher_rate], 'cctv_num':[cctv_num], 'k_remap':[k_remap]})
